...
exports.handler = async (event, context) => {
    ...
    const fill_percent = Math.round((1 - (distance_mm / max_hoehe_mm)) * 100);
    const timestamp = new Date().toISOString();
    const result = {
        thingName: sensorData.thingName,
        distance: parseFloat(sensorData.distance),
        pressure: parseFloat(sensorData.pressure),
        batteryVoltage: parseFloat(sensorData.batteryVoltage),
        batteryFillPercentage: parseFloat(sensorData.batteryFillPercentage),
        fill_percent,
        timestamp
    };
    ...
    const key = `iot-data/${sensorData.thingName}/${Date.now()}.json`;
